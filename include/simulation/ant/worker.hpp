...
struct WorkerUpdater
{
	static void findMarker(Ant& ant, World& world, sim::Context& context)
	{
		...
		for (uint32_t i(sample_count); i--;) {
			...
			if (context.rng.proba(ant.liberty_coef)) {
				break;
			}
		}
		...
		if (ant.phase == Mode::ToFood && result.max_repellent && !result.found_permanent) {
			if (context.rng.proba(repellent_prob_factor * (1.0f - result.max_intensity / to<float>(Conf::MARKER_INTENSITY)))) {
				ant.direction.addNow(context.rng.getUnder(2.0f * PI));
		...
		if (result.max_intensity) {
			...
			if (context.rng.proba(0.2f) && ant.phase == Mode::ToFood) {
	...
	static void update(Ant& ant, World& world, sim::Context& context)
	{
		...
		cell.addPresence();
		ant.search_markers.update(context.dt);
		if (ant.direction_update.updateAutoReset(context.dt)) {
			if (ant.search_markers.ready()) {
				findMarker(ant, world, context);
				ant.direction += context.rng.getFullRange(ant.direction_noise_range);
			}
			else {
				// Fleeing from repellent
				cell.degrade(ant.col_id, Mode::ToFood, 0.25f);
				ant.direction += context.rng.getFullRange(2.0f * ant.direction_noise_range);
			}
		}
		// Add marker
		if (ant.marker_add.updateAutoReset(context.dt)) {
			ant.addMarker(world);
		}
	}
};
